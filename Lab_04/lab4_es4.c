...
void Print(Trt *t[], int l,int r, choise type){
    /*Funzione per la stampa
                                                     *Si passano alla funzione due estremi led r del vettore di punatori;
...
void Operazioni(Trt tratte[],int dim){
    ord ord_corrente=null;          // tiene traccia dell'ordinamento corrente
    int repeat=1,l,r,signal;
    char key[size];                 // chiave della funzione di ricerca
    char comando[size];             // comando per la scelta dal menu
    Trt *vet[dim];      // vettore di puntatori alle struct

    // Inizializzazione del vettore di puntatori alle singole strutture
    for (int i = 0; i < dim; i++)
        vet[i]=&tratte[i];
        ...
        switch (Command(comando)) {
            ...
            case search:                            // Ricerca (parziale o totale)
                ...
                if(ord_corrente==partenza) {        // ordinamento corrente corretto
                    signal = BinSearch(vet, dim, &l, &r, key);      // ricerca binaria
                    if(signal==0)                   // non è stata trovata la chiave (non c'è nulla da stampare)
                        Print(vet,l,r,print_v);
                }
                else
                    LinearSearch(vet, dim,key);                     // ricerca lineare
                break;
